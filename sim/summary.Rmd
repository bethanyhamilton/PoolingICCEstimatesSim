...
```{r}
max(perf_crit$bias_icc_mcse)



max(perf_crit$rel_bias_icc_mcse)
...
perf_crit2  |> ggplot(aes(x = factor(var_combo_graph), y = rel_bias_clus)) +
  ...
  facet_grid(cols = vars(tau)
             , 
             labeller = label_bquote(cols = tau[rho] ==.(tau))) +
  labs(x = "ICC Parameter", y = "RPB") +
  theme_bw() +
  theme(legend.position = "none",
        axis.text.x = element_text(size = 6.5),
...
perf_crit_agg |>
  filter(var_combo_graph < 0.2) |>
  ggplot(aes(var_combo_graph, skewness_mean, col=variance, shape = model)) +
...
perf_crit_agg |>
  filter(var_combo_graph < 0.2) |>
  ggplot(aes(var_combo_graph, kurtosis_mean, col=variance, shape = model)) +
  facet_grid(tau ~ icc_est_n, 
             labeller = label_bquote(rows = tau == .(tau), 
                                            cols = k ==.(icc_est_n))) +
  geom_point( alpha=0.75 ) +
